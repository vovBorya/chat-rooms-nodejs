...
const joinRoom = (socket, roomName) => {
  ...
  socket.emit('joinResult', {roomName});
  socket.broadcast.to(roomName).emit('message', {
    text: `${nickNames[socket.id]} has jointed to ${roomName}.`,
  });

  const roomUsers = io.socket.clients(roomName);

  // Should be refactored
  if (roomUsers.length > 1) {
    let roomUsersCount = `Users currently in ${roomName}:`;
